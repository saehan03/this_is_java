...
//    this.clearInputBox();
  ...
  insertData(data) {
    // 1. 화면에서는 JSON 데이터를 서버 URL과 Method POST로 전송하는 JQUERY AJAX를 구현해야한다.
    // 2. 웹서버에서는 서버 URL 와 Method POST 와 JSON 데이터를 받아들이는 컨트롤러를 구현해야 한다.
    // 3. 웹서버 컨트롤러는 DataBase Service 에 insert 하는 동작을 실행해야 한다.
    // 4. Data Service 는 insert 하는 mybatis insert 메소드를 실행해야 한다.
    // 5. mybatis insert 메소드는 SQL INSERT query 를 데이터베이스 연결한 컨넥션풀에서 실행해야 한다.
    // 6. 실행할 결과를 역순으로 화면까지 리턴해야한다.
   	$.ajax({
